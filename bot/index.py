...
def yandex_art(message):
    ...
    if result.status_code != 200:
        ...
        bot.reply_to(message, "Не могу сгенерировать изображение")
        return None
    result = result.json()
    while True:
        ...
        bot.reply_to(message, "Шедевр рисуется.", disable_notification=True)
        if result['done']:
            encoded = result['response']['image']
            data = base64.b64decode(encoded)
            bot.send_photo(message.chat.id, telebot.types.InputFile(io.BytesIO(data)), reply_parameters=telebot.types.ReplyParameters(message.message_id, message.chat.id, allow_sending_without_reply=True))
            break
